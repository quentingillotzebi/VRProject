...
namespace FLOW
{
	...
	public class FlowShaderBundle : ScriptableObject
	{
		...
		private static void AutoSwitch()
		{
			...
			if (lastAutoSetPipe != pipe)
			{
				...
				var modified = false;
				var guids    = UnityEditor.AssetDatabase.FindAssets("t:" + typeof(FlowShaderBundle).Name);

				foreach (var guid in guids)
				{
					var path   = UnityEditor.AssetDatabase.GUIDToAssetPath(guid);
					var bundle = UnityEditor.AssetDatabase.LoadAssetAtPath<FlowShaderBundle>(path);
				...
				if (modified == true)
				{
					Debug.Log(typeof(FlowShaderBundle).Name + " changed shaders to pipeline: " + pipe);
				}
			}
...
#if UNITY_EDITOR
	[CustomEditor(typeof(FlowShaderBundle))]
	public class FlowShaderBundle_Editor : Editor
	{
		private FlowShaderBundle tgt;

		public override void OnInspectorGUI()
		{
			tgt = (FlowShaderBundle)target;
		...
		private void DrawShader()
		{
			...
			{
				FlowShaderBundle.TrySwitchAllTo(pipe);
			...
			EditorGUI.BeginDisabledGroup(true);
				foreach (var guid in guids)
				{
					...
					if (bundle != null)
					{
						...
						EditorGUILayout.ObjectField(title, bundle, typeof(FlowShaderBundle), false);
	...
	public class FlowShaderBundleDetector : AssetPostprocessor
	{
		...
		private void TryCompileFast(string end)
		{
			...
			if (shaderPath.EndsWith(end) == true)
			{
				...
				var bundle     = AssetDatabase.LoadAssetAtPath<FlowShaderBundle>(bundlePath);
